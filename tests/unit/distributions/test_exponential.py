...
class TestExponentialInitialization:
    ...
    def test_repr_method(self):
        ...
        repr_str = repr(dist)
        assert repr_str == f"Exponential(loc={dist.loc}, rate={dist.rate})"
...
class TestExponentialPDF:
    ...
    def test_pdf_properties_for_array_input(self, loc, rate, x):
    ...
    def test_pdf_properties_for_scalar_input(self, loc, rate, x):
        """Tests that for a scalar input, the PDF returns a non-negative scalar."""

        dist = Exponential(loc, rate)
...
class TestExponentialLPDF:
    ...
    @given(loc=st_loc, rate=st_rate, x=arrays(np.float64, st.integers(0, 10), elements=st.floats(-1e6, 1e6)))
    def test_lpdf_return_type_and_shape_for_array_input(self, loc, rate, x):
    ...
    @given(loc=st_loc, rate=st_rate, x=st.floats(-1e6, 1e6))
    def test_lpdf_return_type_and_shape_for_scalar_input(self, loc, rate, x):
...
class TestExponentialPPF:
    ...
    def test_ppf_return_type_and_shape_for_array_input(self, loc, rate, p):
    ...
    def test_ppf_return_type_and_shape_for_scalar_input(self, loc, rate, p):
...
class TestExponentialGradients:
    ...
    def test_dlog_loc_numerical_for_array_input(self, loc, rate, x):
    ...
    def test_dlog_loc_for_scalar_input(self, loc, rate, x):
    ...
    @given(loc=st_loc, rate=st_rate, x=arrays(np.float64, st.integers(1, 10), elements=st.floats(1e-3, 1e3)))
    def test_dlog_rate_numerical_for_array_input(self, loc, rate, x):
    ...
    @given(loc=st_loc, rate=st_rate, x=st.floats(1e-3, 1e3))
    def test_dlog_rate_for_scalar_input(self, loc, rate, x):
    ...
    @given(loc=st_loc, rate=st_rate, x=st.floats(1e-3, 1e3))
    def test_log_gradients_for_scalar_input(self, loc, rate, x):
