...
@pytest.fixture()
def parametrized_weibull_setup(mocker) -> tuple[Weibull, PipelineState, Optimizer]:
    """
    ...
    """
    component = Weibull(shape=2.0, loc=0.0, scale=1.0)
    ...
    return component, state, optimizer
...
def test_q_function_weibull_raises_value_error_if_h_is_none(parametrized_weibull_setup):
    ...
    weibull_component, state, mock_optimizer = parametrized_weibull_setup
    state.H = None
...
def test_q_function_weibull_returns_correct_types(parametrized_weibull_setup):
    ...
    weibull_component, pipeline_state, mock_optimizer = parametrized_weibull_setup
...
def test_q_function_weibull_respects_params_and_calls_optimizer_correctly(
    ...
    mocker.patch(
        "pysatl_mpest.estimators.iterative._strategies.q_function_strategy.dispatch",
...
def test_q_function_weibull_handles_invalid_loc_for_scale_calculation(parametrized_weibull_setup):
    ...
    weibull_component, pipeline_state, _ = parametrized_weibull_setup
...
def test_weibull_scale_fallback_zero_weighted_sum():
    ...
    X = np.array([val_close_to_loc], dtype=np.float64)
    H = np.array([[1.0, 0.0]], dtype=np.float64)

    original_scale = 5.5
    comp = Weibull(shape=2.0, loc=0.0, scale=original_scale)
