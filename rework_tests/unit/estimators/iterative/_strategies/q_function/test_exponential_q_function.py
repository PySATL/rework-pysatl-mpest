...
@pytest.fixture
def exponential_component_float32() -> Exponential:
    """Fixture that creates an Exponential component with dtype float32."""
    return Exponential(loc=0.0, rate=1.0, dtype=np.float32)
...
def test_exponential_strategy_returns_correct_dtype(exponential_component_float32, pipeline_state_float32):
    """Tests that the analytical strategy for Exponential preserves the component's dtype."""
    block = OptimizationBlock(
        component_id=0, params_to_optimize={"loc", "rate"}, maximization_strategy=MaximizationStrategy.QFUNCTION
    )

    _, new_params_dict = q_function_strategy(
        exponential_component_float32, pipeline_state_float32, block, optimizer=None
    )
