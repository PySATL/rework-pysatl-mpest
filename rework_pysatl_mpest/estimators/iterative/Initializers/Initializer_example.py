...
initializer = ClusterizeInitializer(is_accurate=True, is_soft=False, clusterizer=kmeans)
...
try:
    ...
    print(
        f"  Was: loc=0.0, rate=0.1 -> Now: loc={accurate_mixture.components[0].loc:.3f}, "
        f"rate={accurate_mixture.components[0].rate:.3f}"
    )
    print(
        f"  Was: loc=10.0, rate=0.01 -> Now: loc={accurate_mixture.components[1].loc:.3f}, "
        f"rate={accurate_mixture.components[1].rate:.3f}"
    )
...
fast_initializer = ClusterizeInitializer(
    is_accurate=False, is_soft=False, clusterizer=KMeans(n_clusters=2, random_state=42, n_init=10)
)
...
try:
    ...
    print("\nParameters were recalculated:")
    print(
        f"  Was: loc=0.0, rate=0.1 -> Now: loc={fast_mixture.components[0].loc:.3f}, "
        f"rate={fast_mixture.components[0].rate:.3f}"
    )
    print(
        f"  Was: loc=10.0, rate=0.01 -> Now: loc={fast_mixture.components[1].loc:.3f}, "
        f"rate={fast_mixture.components[1].rate:.3f}"
    )
...
if accurate_mixture is not None and fast_mixture is not None:
    ...
    original_mixture = MixtureModel([Exponential(loc=0.0, rate=0.1), Exponential(loc=10.0, rate=0.01)], [0.5, 0.5])
    ...
    ax.plot(
        ...
        "g--",
        linewidth=3,
        label="Accurate initialization",
        alpha=0.8,
    )
    ax.plot(
        x_plot, [fast_mixture_plot.pdf(xi) for xi in x_plot], "b:", linewidth=3, label="Fast initialization", alpha=0.8
    )
