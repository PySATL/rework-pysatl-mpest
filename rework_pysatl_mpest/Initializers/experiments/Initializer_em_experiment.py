...
from rework_pysatl_mpest.estimators.iterative.steps.block import OptimizationBlock, MaximizationStrategy
...
def create_true_mixture():
    return MixtureModel(
        [
            Exponential(loc=2.25, rate=1.0),
            Exponential(loc=2.5, rate=0.5),
            Exponential(loc=2.75, rate=0.67)
        ],
        [1 / 3, 1 / 3, 1 / 3]
    )
...
def clusterize_initialization(X: np.ndarray, mixture: MixtureModel, is_accurate: bool, is_soft: bool, clusterizer: Any):
    ...
    mixture = initializer.perform(
        ...
        estimation_info=[EstimationStrategy.QFUNCTION,
...
def run_comprehensive_pipeline_experiment(n_samples=1000, max_iterations=10000, run_id=0):
    ...
    try:
        ...
        start_time = time.time()
        initialization_methods['Kmeans'] = clusterize_initialization(X, random_mixture, is_accurate=False,
                                                                     is_soft=False, clusterizer=kmeans)
    ...
    try:
        ...
        start_time = time.time()
        initialization_methods['Cmeans'] = clusterize_initialization(X, random_mixture, is_accurate=False, is_soft=True,
                                                                     clusterizer=cmeans)
...
def plot_comparison_results(aggregated_df_for_n, X_sample, true_mixture, final_mixtures, n_samples):
    ...
    ax1.hist(X_sample, bins=50, density=True, alpha=0.3, color='gray',
    ...
    for method_name, final_mixture in final_mixtures.items():
        if final_mixture is not None:
            ...
            ax1.plot(x_plot, fitted_pdf, '--', linewidth=2,
...
if __name__ == "__main__":
    ...
    for n_samples in sample_sizes:
        print(f"\nЭКСПЕРИМЕНТ ДЛЯ {n_samples} СЭМПЛОВ")
    ...
    print("Результаты сохранены в папке em_pipeline_results/")